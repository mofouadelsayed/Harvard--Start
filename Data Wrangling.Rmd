...
```{r}
# Fuctions applied to simple vectors
...
- HTML: Hyper Text Markup Language. "rvest" is the package inside "tidyverse" that reades data from web pages. In the following example we will see how data for the "murders" dataset was imported from Wikipedia.
```{r}
library(rvest)
...
```{r}
tab<- tab %>% setNames(c("State", "Population", "Total", "Murders", "Gun_Murders", "Gun_Ownership", "Total_Rate", "Gun_Murder_Rate"))
head(tab)
```
