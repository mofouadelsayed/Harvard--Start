...
# To separate the column names only (headers)
col_names<- x[[1]]
x<- x[-1]                                             # This defines the data imported without the header
```

- In order to extract the column number in the second argument of the function `map()`. But to force `map()` to return characters i can use `map_chr()` instead & use `map_int()` if i want integers.
```{r}
map(x,1) %>% head
...
- ALL the above steps could be replaced by the following which returns a matrix instead of a list
```{r}
...
the_names_1
the_names_1<- the_names_1 %>% str_replace_all(",\\s.", "")             # str_replace only replaces the first column
the_names_1
...
- To see how many Variables have long names (>12 characters). The `distinct()` function displays only the distinct values of the variable entered, without it it would display around 180 values all for the same 4 countries. 
```{r}
...
**Exploring On My Own**
- I'll try to import data from my weekly GR Inventory report to Rstudio & Analyze
```{r}
# To list the files in a specific location. NOTICE the "\", if i used the opposite one it won't work
...
body %>% filter(Length=="19'04\"") %>% ggplot(aes(Grade, `Held Total`, col= Grade)) + geom_bar(stat = "identity") +  theme(axis.text.x = element_text(angle=90, hjust=1))
>>>>>>> 962285008edec455908e451523e7b80cfc0ee9be
```
