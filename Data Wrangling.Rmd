...
- *String Splitting*. Ususally when we import a ".csv" file using `read_csv()` or `read.csv()` R separates values of each row separated by columns into columns. But to illustrate the uses of `str_split()` function we will assume we imported data with a different function `readLines()` that imports data is tables into lines of contiuous strings separated by a comma. Then we will try to separate or "split" those strings to make a data frame.
```{r}
...
- In order ti extract the column number in the second argument of the function `map()`. But to force `map()` to return characters i can used `map_chr()` instead & use `map_int()` if i want integers.
```{r}
map(x,1) %>% head
...
- WE want to extract data from table S1 found on the PDF published on this website <http://www.pnas.org/content/112/40/12349.abstract>. Here's how 
```{r}
library(pdftools)
